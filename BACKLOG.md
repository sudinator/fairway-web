...
## Shipped
- v1.53.1 — code-review bug fixes: (1) **dates off by one** — `fmtDate` now parses bare `YYYY-MM-DD` (`played_at`) as LOCAL, so Rounds/round detail/share card/admin no longer show the day before in negative-UTC zones; live view routed through `fmtDate` too. (2) **lock-time flush** now backs up + pushes `penalties` and `sand` (was dropping them, and overwriting the good backup, if the screen locked mid-write). (3) **duplicate game rounds** — migration 0043 de-dupes and adds a UNIQUE index on `rounds(game_id,user_id)`; 0044 makes `post_game_rounds` ON CONFLICT-safe. (4) **recorded round date** now uses the game's `played_at` match date, not `created_at` (client + 0044). (5) **push retries** carry `clock_start/clock_end` so the round clock isn't lost on a failed-then-retried write. Plus: admin round view uses robust `allocateStrokes` for received-stroke dots.

...
3. Require GitHub `CI / verify`, Robustness, Vercel, and the manual Staging integration workflow to pass. `CI / verify` reconstructs a disposable database and runs the structural plus negative authorization tests. The live Production schema guard intentionally does not run on Staging.
...
2. The trusted push to `main` runs Production migration parity, the live read-only Production schema guard, and normal CI.

...
checks = {
    'alternate shot collapses duplicated partner rows into side columns': 'Collapse each foursome to exactly TWO scoring entities' in score and 'const makeSide = (ids: string[], which: "a" | "b")' in score,
    ...
    'alternate shot modal writes only the side score': 'if (sideCol?.altSide) { onSetHole(p.id, edit.holeIdx, { strokes: patch.strokes }); }' in score,

...
export type StatKey = "fir" | "gir" | "putts";
export type Band = { lo: number; hi: number; avg: number };
export type HcpBand = { hcp: number; fir: Band; gir: Band; putts: Band };

// +1 = higher is better (fairways, greens); -1 = lower is better (putts).
export const BENCH_DIR: Record<StatKey, 1 | -1> = { fir: 1, gir: 1, putts: -1 };
export const BENCH_LABEL: Record<StatKey, string> = { fir: "Fairways", gir: "Greens in reg.", putts: "Putts / round" };
export const BENCH_UNIT: Record<StatKey, string> = { fir: "%", gir: "%", putts: "" };
// Track layout domains (min,max) so a value/band maps to a 0–100% position.
export const BENCH_DOMAIN: Record<StatKey, [number, number]> = { fir: [20, 75], gir: [0, 75], putts: [28, 41] };

export const BENCH: HcpBand[] = [
  // scratch — averages sourced (FW ~56%, GIR ~56%, putts 31.3); ranges estimated.
  { hcp: 0,  fir: { lo: 46, hi: 67, avg: 56 }, gir: { lo: 40, hi: 72, avg: 56 }, putts: { lo: 28, hi: 34, avg: 31.3 } },
  // 5 — averages sourced (FW ~51%, GIR ~46%, putts 32.5); ranges estimated.
  { hcp: 5,  fir: { lo: 42, hi: 62, avg: 51 }, gir: { lo: 30, hi: 60, avg: 46 }, putts: { lo: 29, hi: 35, avg: 32.5 } },
  // 10 — averages + ranges sourced (Break X): FW 49% (35–71), GIR 37% (22–54), putts 33.9 (30–37).
  { hcp: 10, fir: { lo: 35, hi: 71, avg: 49 }, gir: { lo: 22, hi: 54, avg: 37 }, putts: { lo: 30, hi: 37, avg: 33.9 } },
  // 15 — averages + ranges sourced (Break X): FW 48%, GIR 26% (5–44), putts 34.8 (31–39).
  { hcp: 15, fir: { lo: 33, hi: 63, avg: 48 }, gir: { lo: 5,  hi: 44, avg: 26 }, putts: { lo: 31, hi: 39, avg: 34.8 } },
  // 20 — averages sourced (FW ~43%, GIR ~22%, putts 36.1); ranges estimated.
  { hcp: 20, fir: { lo: 30, hi: 56, avg: 43 }, gir: { lo: 8,  hi: 38, avg: 22 }, putts: { lo: 32, hi: 40, avg: 36.1 } },
];
...
export function bandFor(hcpRaw: number): HcpBand {
  ...
  return { hcp: hcpRaw, fir: band("fir"), gir: band("gir"), putts: band("putts") };
}

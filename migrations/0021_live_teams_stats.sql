-- 0021_live_teams_stats.sql
-- Extends the public live scorecard so the read-only page can show team scores
-- (with member names) and per-player stats. Re-creates get_live_scorecard with
-- two additions: the named `teams` list on the game, and `penalties`/`sand` on
-- each player (for the stats strip). Everything else is identical to 0018 —
-- still alias-keyed, still no account ids / emails / phones / GHIN / handicap index.

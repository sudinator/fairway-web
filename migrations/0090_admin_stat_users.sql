...
begin
  ...
  if p_stat = 'users_total' then
    return query select coalesce(p.display_name,'(no name)'), coalesce(p.email,''), null::text
      from profiles p where coalesce(p.is_test,false)=false and coalesce(p.deactivated,false)=false
    ...
    return query select coalesce(p.display_name,'(no name)'), 'first seen '||to_char(fa.first_day,'Mon DD'), 'new'::text
      from profiles p join (select user_id, min(day) first_day from daily_active group by user_id) fa on fa.user_id=p.id
    ...
    return query select coalesce(p.display_name,'(no name)'), 'last seen '||to_char(mx.last_day,'Mon DD'), 'lapsed'::text
      from profiles p join (select user_id, max(day) last_day from daily_active group by user_id) mx on mx.user_id=p.id
      where coalesce(p.is_test,false)=false and coalesce(p.deactivated,false)=false
    ...
    return query select coalesce(p.display_name,'(no name)')||' · '||coalesce(r.course,'course'),
      ...
      from rounds r join profiles p on p.id=r.user_id where r.played_at = d order by r.created_at;
    ...
    return query select coalesce(p.display_name,'(no name)'), coalesce(p.email,''), 'installed'::text
      from profiles p where coalesce(p.is_test,false)=false and coalesce(p.deactivated,false)=false and p.last_standalone is true
      order by p.display_name nulls last;

  elsif p_stat = 'browser' then
    return query select coalesce(p.display_name,'(no name)'), coalesce(p.email,''), 'browser'::text
      from profiles p where coalesce(p.is_test,false)=false and coalesce(p.deactivated,false)=false and p.last_standalone is false
      order by p.display_name nulls last;
    ...
    return query select coalesce(p.display_name,'(no name)'), 'card visible', 'on'::text
      from profiles p where coalesce(p.is_test,false)=false and coalesce(p.deactivated,false)=false and coalesce(p.show_card,true)=true
      order by p.display_name nulls last;

  elsif p_stat = 'share_off' then
    return query select coalesce(p.display_name,'(no name)'), 'opted out', 'off'::text
      from profiles p where coalesce(p.is_test,false)=false and coalesce(p.deactivated,false)=false and coalesce(p.show_card,true)=false
      order by p.display_name nulls last;

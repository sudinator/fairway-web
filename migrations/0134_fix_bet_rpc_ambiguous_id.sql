...
-- Runtime fix discovered by the real staging integration suite.
--
-- 0133 introduced save_bet_expense_atomic() with RETURNS TABLE(id, created_at).
-- In PL/pgSQL those output names are variables, so bare references such as
-- `where id = p_game` are ambiguous against table columns at runtime.
-- This migration preserves the RPC signature/behavior and qualifies every
-- table column that can collide with output-variable names.
--
-- AUTHORIZATION:
--   save_bet_expense_atomic — game organizer or active group admin (unchanged from 0133).
...
begin
  ...
  if p_event is not null
     and not exists (
       select 1
       from public.group_events as ge
       where ge.id = p_event
         and ge.group_id = p_group
     ) then
  ...
  if exists (
    ...
    where coalesce((x->>'paid_cents')::integer,0) <= 0
      ...
      or (
        nullif(x->>'user_id','') is not null
        and not public.is_group_member(p_group,(x->>'user_id')::uuid)
      )
      or (
        nullif(x->>'guest_id','') is not null
        and not exists (
          select 1
          from public.group_guests as gg
          where gg.id = (x->>'guest_id')::uuid
            and gg.group_id = p_group
        )
      )
      or (
        nullif(x->>'guest_id','') is not null
  ...
  select coalesce(
    (
      select (x->>'user_id')::uuid
      from jsonb_array_elements(p_payers) x
      where nullif(x->>'user_id','') is not null
      limit 1
    ),
    (
      select (x->>'sponsor_user_id')::uuid
      from jsonb_array_elements(p_payers) x
      where nullif(x->>'sponsor_user_id','') is not null
      limit 1
    )
  )
  into v_primary;
  ...
  if p_replace_expense is not null then
    ...
    where e.id = p_replace_expense
      ...
      and e.source_game_id = p_game
      and e.source_kind = 'tgc_bet'
    for update;
  ...
  return query
  select v_id, v_created;
end
$$;

revoke all on function public.save_bet_expense_atomic(
  uuid,uuid,uuid,uuid,text,integer,jsonb,jsonb
) from public;
...
grant execute on function public.save_bet_expense_atomic(
  uuid,uuid,uuid,uuid,text,integer,jsonb,jsonb
) to authenticated;
